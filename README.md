...
# db notes
When user is created server automatically assign mqtt client and pwd.
Admin can create Sensor categorys with different prameters. For example CO2 sensor with prameter co2_level.
Then sensor model can be assigned to that category. That allows multiple sensors to all have the same category, to be the same type of sensor.
...
It is published by multilpe clients to client specific topic. For example user1/building_1/sensors/temperature
Clients are connected to the MQTT broker with username and password.

Then there is subscriber script that subscribes to all topic.
Once it gets message from broker based on sensor name it selects appropriate parser script and after cleaning data it saves it to db.
